      ...
      <div class="content-r">
        <div class="custom-page-view-search" ref="search">
          ...
          <div class="current_location">
            当前位置：<span>{{ currentItem.name || '全部' }}</span>
        ...
        <div class="custom-page-view-table" v-if="exhibition">
          <a-form :form="form">
            ...
            <a-form-item :label-col="{ span: 8 }" class="bg" :wrapper-col="{ span: 16 }" label="疾病名称">
              <a-input v-model="currentItem.name" />
            </a-form-item>
            <a-form-item :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" label="疾病介绍">
...
import apiBase from '@/api/apiDicManage'
  ...
  methods: {
    ...
    getParentKey(key, tree) {
      ...
      for (let i = 0; i < tree.length; i++) {
        const node = tree[i]
        if (node.id == key) {
          index = arr.findIndex(item => item.id == node.id) //根据 已知id 获取在数组中的位置(index)；
    ...
    getMenuBtnList(menuTreeList) {
      for (let tmp of menuTreeList) {
        tmp = { ...tmp.item, items: tmp.items || [] }
        tmp.scopedSlots = { title: 'title' }
        if (tmp.items.length > 0) {
          this.getMenuBtnList(tmp.items)
        }
      }
    },
    //设置时间
